...
func GetInstance(padding string) *XTea {
	return &XTea{
		Padding: padding,
	}
}
...
// Encrypt 加密
func (t *XTea) Encrypt(plaintext []byte, key []byte) ([]byte, error) {
...
// Decrypt 解密
func (t *XTea) Decrypt(ciphertext []byte, key []byte) ([]byte, error) {
...
// 补码处理
func (t *XTea) padding(data []byte, blockSize int) ([]byte, error) {
	switch t.Padding {
	case "NoPadding":
...
// 去掉补码处理
func (t *XTea) unpadding(data []byte) ([]byte, error) {
	switch t.Padding {
	case "NoPadding":
