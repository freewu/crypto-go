...
import (
	...
	"encoding/hex"
	"fmt"
	"github.com/freewu/crypto-go/aes"
	"strings"
)
...
func main() {
	...
	fmt.Printf("\n===== CFB模式 %v\n", strings.Repeat("=", 20))
	instance = aes.GetInstance("CFB/PKCS7Padding")
	encrypted, err = instance.Encrypt(data, key, iv)
	if err != nil {
		panic(err)
	}
	...
	fmt.Printf("\n===== OFB模式 %v\n", strings.Repeat("=", 20))
	instance = aes.GetInstance("OFB/PKCS7Padding")
	encrypted, err = instance.Encrypt(data, key, iv)
	if err != nil {
		panic(err)
	}
	...
	fmt.Printf("\n===== CTR模式 %v\n", strings.Repeat("=", 20))
	instance = aes.GetInstance("CTR/PKCS7Padding")
	encrypted, err = instance.Encrypt(data, key, iv)
	if err != nil {
		panic(err)
	}
	...
	fmt.Printf("\n===== ECB模式 %v\n", strings.Repeat("=", 20))
	instance = aes.GetInstance("ECB/PKCS7Padding")
	encrypted, err = instance.Encrypt(data, key, iv)
	if err != nil {
		panic(err)
	}
