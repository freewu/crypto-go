...
func TestStaticKeySize(t *testing.T) {
	// 密钥低于 16位 4位
	_, err := Encrypt([]byte("bluefrog"), []byte("1234"), 64)
	if err == nil || string(err.Error()) != "tea: incorrect key size" {
		t.Errorf("need throw exception tea: incorrect key size")
	}
	// 密钥超过16位 17
	_, err = Encrypt([]byte("bluefrog"), []byte("12345678123456789"), 64)
	if err == nil || string(err.Error()) != "tea: incorrect key size" {
		t.Errorf("need throw exception tea: incorrect key size")
	}
	// 密钥为必须 16位
	_, err = Encrypt([]byte("bluefrog"), []byte("1234567812345678"), 64)
	if err != nil {
		t.Errorf("throw exception %v\n", err)
	}
}

func TestStaticRound(t *testing.T) {
	// 加密轮次不能为奇数次
	_, err := Encrypt([]byte("bluefrog"), []byte("1234567812345678"), 15)
	if err == nil || string(err.Error()) != "tea: odd number of rounds specified" {
		t.Errorf("need throw exception tea: odd number of rounds specified")
	}
...
func TestStaticEncryptDecrypt(t *testing.T) {
	// 加密
	chipertext, err := Encrypt([]byte("bluefrog"), []byte("1234567812345678"), 64)
	if err != nil {
		t.Errorf("throw exception %v\n", err)
	}
...
func TestStaticEncryptDecryptPadding(t *testing.T) {
	...
	// 加密
	chipertext, err := Encrypt(pack, []byte("1234567812345678"), 64)
	if err != nil {
		t.Errorf("throw exception %v\n", err)
	}
	fmt.Printf("tea encrypt hex: %v\n", hex.EncodeToString(chipertext))
